...
//					$scope.sData=$scope.sData+","+$scope.example14data[$scope.example14model[($scope.example14model.length)-1].id-1].label;
		...
		$scope.feedbackNameValid = false;
		$scope.feedbackNameSuccess = false;
		$scope.feedbackNameError = false;
		$scope.feedbackNameFeedback = "";
		
		$scope.validFeedbackName = function(valid){
			$scope.feedbackNameValid = valid;
			if($scope.feedback.feedbackName != undefined) {
				if($scope.feedback.feedbackName.length <= 0) {
					$scope.feedbackNameError = true;
					$scope.feedbackNameFeedback = "has-error has-feedback";
				} else {
					$scope.feedbackNameSuccess = true;
					$scope.feedbackNameError = false;
					$scope.feedbackNameFeedback = "has-success has-feedback";
				}
			} else {
				$scope.feedbackNameError = true;
				$scope.feedbackNameSuccess = false;
		...
		$scope.feedbackMailValid = false;
		$scope.feedbackMailSuccess = false;
		$scope.feedbackMailError = false;
		$scope.feedbackMailFeedback = "";
		
		$scope.validFeedbackMail = function(valid){
			$scope.feedbackMailValid = valid;
			if($scope.feedback.feedbackMail != undefined) {
				if($scope.feedback.feedbackMail.length <= 0) {
					$scope.feedbackMailError = true;
					$scope.feedbackMailFeedback = "has-error has-feedback";
				} else {
					$scope.feedbackMailSuccess = true;
					$scope.feedbackMailError = false;
					$scope.feedbackMailFeedback = "has-success has-feedback";
				}
			} else {
				$scope.feedbackMailError = true;
				$scope.feedbackMailSuccess = false;
		...
		$scope.feedbackPhoneValid = false;
		$scope.feedbackPhoneSuccess = false;
		$scope.feedbackPhoneError = false;
		$scope.feedbackPhoneFeedback = "";
		
		$scope.validFeedbackPhone = function(valid){
			$scope.feedbackPhoneValid = valid;
			if($scope.feedback.feedbackPhone != undefined) {
				if($scope.feedback.feedbackPhone.length <= 0) {
					$scope.feedbackPhoneError = true;
					$scope.feedbackPhoneFeedback = "has-error has-feedback";
				} else {
					$scope.feedbackPhoneSuccess = true;
					$scope.feedbackPhoneError = false;
					$scope.feedbackPhoneFeedback = "has-success has-feedback";
				}
			} else {
				$scope.feedbackPhoneError = true;
				$scope.feedbackPhoneSuccess = false;
		...
		$scope.feedbackMsgValid = false;
		$scope.feedbackMsgSuccess = false;
		$scope.feedbackMsgError = false;
		$scope.feedbackMsgFeedback = "";
		
		$scope.validFeedbackMsg = function(valid){
			$scope.feedbackMsgValid = valid;
			if($scope.feedback.feedbackMsg != undefined) {
				if($scope.feedback.feedbackMsg.length <= 0) {
					$scope.feedbackMsgError = true;
					$scope.feedbackMsgFeedback = "has-error has-feedback";
				} else {
					$scope.feedbackMsgSuccess = true;
					$scope.feedbackMsgError = false;
					$scope.feedbackMsgFeedback = "has-success has-feedback";
				}
			} else {
				$scope.feedbackMsgError = true;
				$scope.feedbackMsgSuccess = false;
		...
		$scope.addFeedback = function() {
			var promise = restAPIService.feedbacksService().save(null,$scope.feedback);
			promise.$promise.then(
					function (response) {
						dialogs.notify("Success", response.success, {'size': 'sm' });
						$scope.feedback = {
