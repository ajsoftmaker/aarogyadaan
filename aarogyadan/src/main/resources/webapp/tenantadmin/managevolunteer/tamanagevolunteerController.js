...
angular.module('aarogyadanWebApp').controller('tamanagevolunteer',
		tamanagevolunteerController);

function tamanagevolunteerController($scope, $rootScope, $state, dialogs,
...
//	getBatches();
	...
	function getVolunteers() {
		var promise1 = restAPIService.volunteersService().query();
	...
	$scope.onEdit = function(tenant) {
		...
		$scope.mode = "edit";
		$scope.id = tenant.id;
		$state.go("home.tamanagetenants.taaddvolunteer");
	}

	$scope.addNewVolunteer = function() {
		$scope.parent = false;
		$scope.mode = "add";
		$state.go("home.tamanagevolunteer.taaddvolunteer");
	}
	...
	$scope.onDisable = function(tenant) {
		...
		dlg.result.then(function(btn) {
			...
			promise.$promise.then(function(response) {
				...
				getTenants();
				$state.go('home.tamanagevolunteer');
			}, function(error) {
				dialogs.error("Error", error.data.error, {'size' : 'sm'});
			});
		}, function(btn) {
