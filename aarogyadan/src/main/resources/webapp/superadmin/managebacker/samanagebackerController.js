...
angular.module('aarogyadanWebApp').controller('samanagebackerController',
		samanagebackerController);

function samanagebackerController($scope, $rootScope, $state, dialogs,
...
//	getBatches();
	...
	function getVolunteers() {
		var promise1 = restAPIService.backersService().query();
	...
	$scope.onEdit = function(tenant) {
		...
		$scope.mode = "edit";
		$scope.id = tenant.id;
		$state.go("home.samanagebacker.saaddbacker");
	}

	$scope.addNewBacker = function() {
		$scope.parent = false;
		$scope.mode = "add";
		$state.go("home.samanagebacker.saaddbacker");
	}
	...
	$scope.onDisable = function(tenant) {
		...
		dlg.result.then(function(btn) {
			...
			promise.$promise.then(function(response) {
				...
				getTenants();
				$state.go('home.samanagevolunteer');
			}, function(error) {
				dialogs.error("Error", error.data.error, {'size' : 'sm'});
			});
		}, function(btn) {
