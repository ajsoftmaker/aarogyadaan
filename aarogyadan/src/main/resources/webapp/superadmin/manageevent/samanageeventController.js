...
.controller('samanageeventController', samanageeventController);

function samanageeventController($scope, $rootScope, $state, dialogs,
...
//	getBatches();
	...
	function getEvents() {
		var promise1 = restAPIService.eventsService().query();
	...
	$scope.onEdit = function(tenant) {
		...
		$scope.mode = "edit";
		$scope.id = tenant.id;
		$state.go("home.samanageevent.saaddevent");
	}
