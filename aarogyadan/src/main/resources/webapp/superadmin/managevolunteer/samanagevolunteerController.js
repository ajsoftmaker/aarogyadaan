...
angular.module('aarogyadanWebApp').controller('samanagevolunteer',
		samanagevolunteerController);

function samanagevolunteerController($scope, $rootScope, $state, dialogs,
...
//	getBatches();
	...
	$scope.onEdit = function(tenant) {
		...
		$scope.mode = "edit";
		$scope.id = tenant.id;
		$state.go("home.samanagetenants.saaddvolunteer");
	}

	$scope.addNewVolunteer = function() {
		$scope.parent = false;
		$scope.mode = "add";
		$state.go("home.samanagevolunteer.saaddvolunteer");
	}
